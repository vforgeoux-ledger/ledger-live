...
describe("Send flow", () => {
  ...
  it.each(testAccounts.map(account => [account.currency.name, account]))(
    ...
    async (_currency, account: Account) => {
      ...
      await common.performSearch(getDefaultAccountNameForCurrencyIndex(account));
      ...
      await operationDetailsPage.checkAccount(getDefaultAccountNameForCurrencyIndex(account));
